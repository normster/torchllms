"""FlashAttention with learned sinks and banded (sliding window) attention.

Copied from gpt_oss/triton/attention.py to avoid runtime dependency on the
gpt-oss package. Only dependency is triton >= 3.4.

Provides:
- attention()      — Triton FlashAttention kernel (prefill, seqlen >= 64)
- attention_ref()  — Eager reference implementation (decode / short seqlen)

Both support per-head learned sink tokens and optional sliding window.
"""
...
def attention_ref(
    ...
    """Eager reference attention with sink tokens. Used for decode (seqlen=1)
    and as a correctness reference for the triton kernel.

    Args:
        query:  [batch, num_queries, n_kv_heads, q_per_kv, head_dim]
        key:    [batch, num_keys, n_kv_heads, head_dim]
        value:  [batch, num_keys, n_kv_heads, head_dim]
        sinks:  [n_heads] per-head sink logit biases
        sm_scale: softmax scale (1/sqrt(head_dim))
        sliding_window: window size (None or 0 = no sliding window)
        start_q: position offset for queries (for KV cache)
    Returns:
        [batch, num_queries, n_heads * head_dim]
    """
    ...
    # start_q may be 2D [bsz, 1] from input_pos[:, 0:1]; squeeze to scalar
    # so that pos_queries stays 1D and the causal mask is 2D.
    if isinstance(start_q, torch.Tensor):
        start_q = start_q.squeeze()
